...
import { Shield, ArrowUpRight, Flame } from 'lucide-react';
...
const HomeTab: React.FC<HomeTabProps> = ({ isDark, setActiveTab }) => {
  return (
    <div className="relative min-h-screen flex flex-col items-center justify-center text-center px-4 overflow-hidden pt-24 md:pt-0">
      ...
      <div className="max-w-5xl z-20">
        ...
        <h1 className={`text-4xl md:text-[7.5rem] font-medium tracking-tighter mb-8 leading-[0.9] transition-colors duration-700 ${isDark ? 'text-white' : 'text-black'}`}>
          One-click for <br />
          <span className={`bg-gradient-to-r bg-clip-text text-transparent transition-all duration-700 ${isDark ? 'from-white via-white/80 to-white/40' : 'from-black via-black/80 to-black/40'
            }`}>Asset Defense</span>
        </h1>

        <p className={`text-sm md:text-xl max-w-2xl mx-auto mb-14 leading-relaxed font-light transition-colors ${isDark ? 'text-gray-400' : 'text-gray-600'
          }`}>
